...
export interface HowToStudyMobileSection {
  ...
  /** Full section markdown extracted from the manuscript */
  contentMarkdown: string;
}

/** Section IDs and headings aligned with howToStudyNav.ts */
const SECTION_CONFIG: Array<Omit<HowToStudyMobileSection, 'contentMarkdown'>> = [
  {
...
function resolveSectionContent(targetHeading: string): string {
  return getHowToStudySectionMarkdown(targetHeading) ?? '';
}

export const HOW_TO_STUDY_MOBILE_SECTIONS: HowToStudyMobileSection[] = SECTION_CONFIG.map(
  (section) => ({
    ...section,
    contentMarkdown: resolveSectionContent(section.targetHeading),
  }),
);
...
export function getHowToStudySectionNeighbors(id: string): {
  previous?: HowToStudyMobileSection;
  next?: HowToStudyMobileSection;
} {
  const index = HOW_TO_STUDY_MOBILE_SECTIONS.findIndex((section) => section.id === id);
  if (index === -1) return {};

  return {
    previous: index > 0 ? HOW_TO_STUDY_MOBILE_SECTIONS[index - 1] : undefined,
    next:
      index < HOW_TO_STUDY_MOBILE_SECTIONS.length - 1
        ? HOW_TO_STUDY_MOBILE_SECTIONS[index + 1]
        : undefined,
  };
}
